...
// Failed opening required '/var/www/citebank.org/includes//sites/all/modules/citebank_importer/DBInterfaceController.php' (include_path='.:/usr/share/php:/usr/share/pear') in /var/www/citebank.org/includes/common.inc(1697) : eval()'d code on line 11
 ...
 *
 */
if (!defined('CLASS_DBInterfaceController')) {
	define('CLASS_DBInterfaceController', true);
 ...
 *
 */
class DBInterfaceController_stats
{
	private $db; 
	private $i;
...
citestats - CiteBank stats updated Thu Nov 17 01:01:01 CST 2011
	...
	* Total citations: 102180
	* Citations with PDFs from BHL: 1
	* Citations without PDFs attached: 102179
	* Citations from BHL: 1
	* Citations from Scielo: 16
	* Citations from various sources: 0
	* Citations from Pensoft: 0
	* Citations from Zookeys: 0
	* Registerered CiteBank users: 86
*/
...
/*
	* Total citations: 102180
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) WHERE n.type = 'biblio'

	* Citations with PDFs from BHL: 1
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%.pdf';

	* Citations without PDFs attached: 102179
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link NOT LIKE '%.pdf'

	* Citations from BHL: 1
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%biodiversity%'

	* Citations from Scielo: 16
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%scielo%'

	* Citations from various sources: 0
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%handle.net%'

	* Citations from Pensoft: 0
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%pensof%'


	* Citations from Zookeys: 0
SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%zookey%'
...
$sql = "SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%biodiversity%' AND b.biblio_label LIKE '%biodiversity%'";
getStat($dbi, $msg, $sql);
...
//$sql = "SELECT COUNT(*) AS total FROM node AS n JOIN biblio AS b ON (n.nid = b.nid) JOIN citebank_unique_identifier as c ON (b.nid = c.nid) WHERE n.type = 'biblio' AND c.link LIKE '%zookey%'";
...
$sql = "SELECT COUNT(*) AS total FROM users WHERE STATUS = 1 AND access > 0";
getStat($dbi, $msg, $sql);
...
function statsEchoDate()
{
	//citestats - CiteBank stats updated Thu Nov 17 01:01:01 CST 2011	
	echo '<br>';
	echo 'citestats - CiteBank stats updated ' .  date('m d H:i:s  Y') . ' 	';
	echo '<br>';
