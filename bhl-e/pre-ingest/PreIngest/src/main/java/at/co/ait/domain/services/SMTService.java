...
public class SMTService extends ProcessbuilderService {
	...
	public DigitalObject map(DigitalObject obj, String params)
		...
		String tmpfile = ConfigUtils.getTmpFileName(obj.getSubmittedFile(),Configuration.getString("SMTService.tmpfilepostifx")); //$NON-NLS-1$
		...
		for (String param : params.split(Configuration.getString("SMTService.smtparam_splitstring"))) { //$NON-NLS-1$
			commands.add(param);
		}
		...
		process(commands);
		obj.setSmtoutput(new File(tmpfile));
