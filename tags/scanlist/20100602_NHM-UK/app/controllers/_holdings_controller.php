  ...
  var $name = 'Holdings';
    ...
    var $uses = array('Holding','Bid','Holding','Status','Title');
     ...
     function viewdirect($id = null) {
        ...
        $myid = $this->Holding->find('`035` like \'%a' . rtrim($catkey) .'\' AND place = \'' . $place . '\'');
        ...
        $mytitlecatkey = $this->Title->find('`catkey` = ' . rtrim($catkey));
			...
			else # must be a fresh record never considered before
			{
				...
				$this->Holding->id = $id;
				$this->set('myholding', $this->Holding->read());
				// 20080902 - added supply of preferred title bib_id for deprecated NHM title as we want to view place any new bid on this, and not the deprecated record.
				//$mybibid = $this->Holding->find('`new_id` = ' . rtrim($catkey));
				//$this->set('mybibid', $mybibid);
