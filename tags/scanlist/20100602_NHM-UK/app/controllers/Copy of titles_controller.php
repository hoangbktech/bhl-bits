...
//**********************
        ...
        function index() {
	           ...
	           /* Setup filters */
	               ...
	               $this->Pagination->init(
	                   $cond, 'Title', NULL, array('catkey', 'title', 'pub', 'date_created'), 0
	               );
	                               //'depreciated is NULL'
	               $this->set('Titles', $this->Title->findAll(
...
//////////////GUTS OF THE LOGIC AS TO WHETHER A TITLE CAN BE ADDED TO A PACKING LIST IS HERE ////////
            ...
            //       iprtitles.cdm_checked_decision != rejected (int 3) & titles.any_pre_1860 = Y (int 1)
             // OR
            //       (
            //         iprtitles.cdm_checked_decision = Accepted (int 2) OR pre-1860 only (int 4)
            //       ) & titles.any_pre_1860 = N (int 2)

            // have a conds variable here to avoid repeating the long syntax

            $conds = '`title` like \'%' . rtrim($titleletters ) . '%\' AND any_scannable_status =\'1\' AND (digitized_title_status !=\'1\' || digitized_title_status is null) AND ipr_status != \'5\' AND ((cdm_checked_decision != \'3\' AND any_pre_1860 = \'1\') OR ((cdm_checked_decision = \'2\' || cdm_checked_decision = \'4\') AND any_pre_1860 = \'2\')) ';
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 function pickupatitle_problemlist($id = null) {
		   ...
		   // this is to try and pull up a record by catkey from titles to be scanned page or tell the user if there is not one that matches.
            ...
            // b) have digitizedtitles.digitized_title_status != Already scanned (int 1) AND
            // c) have iprtitles.ipr_status != No rights permitted (int 5) AND
            // d) have iprtitles.cdm_checked_decision = Accepted (int 2)

            // have a conds variable here to avoid repeating the long syntax

            //$conds = '`title` like \'%' . rtrim($titleletters ) . '%\' AND any_scannable_status =\'1\' AND (digitized_title_status !=\'1\' || digitized_title_status is null) AND ipr_status != \'5\' AND cdm_checked_decision = \'2\' ';
            //var_dump($conds);
            $conds = '`title` like \'%' . rtrim($titleletters ) . '%\' AND any_scannable_status =\'1\' AND (digitized_title_status !=\'1\' || digitized_title_status is null) AND ipr_status != \'5\' AND ((cdm_checked_decision != \'1\' AND any_pre_1860 = \'1\') OR ((cdm_checked_decision = \'2\' || cdm_checked_decision = \'4\') AND any_pre_1860 = \'2\')) ';
